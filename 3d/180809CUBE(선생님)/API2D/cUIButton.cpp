...
/*
void cUIButton::Init(ST_SIZE stSize)
{
...
void cUIButton::Render(HDC hdc)
{
	...
	DrawText(hdc, m_szText, (int)strlen(m_szText),
		&m_aStateRect[m_eButtonState],
		DT_CENTER | DT_VCENTER | DT_NOCLIP | DT_SINGLELINE);
}

bool cUIButton::WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	switch (message)
	{
	case WM_LBUTTONDOWN:
	{
		...
		RECT rc = cUtil::MakeRect(m_ptScreen, m_stSize);
