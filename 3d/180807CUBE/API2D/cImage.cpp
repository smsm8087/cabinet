	...
	, m_sPath("")
{
	...
	m_hMemDC = CreateCompatibleDC(hdc);
	m_hBitmap = (HBITMAP)CreateBitmap(stSize.nWidth, stSize.nHeight, 1, 32, NULL);
	m_hOldBitmap = (HBITMAP)SelectObject(m_hMemDC, m_hBitmap);
	BITMAP bm;
	GetObject(m_hBitmap, sizeof(BITMAP), &bm);
	m_stSize.nWidth = bm.bmWidth;
...
void cImage::Render(cImage * pImage, ST_SIZE stSize)
{
	ST_SIZE sOrgSize = pImage->GetSize();
