...
void cMainGame::Init()
{
	...
	vector<cAirplane*> vecTarget;
	vecTarget.push_back(new cAirplane());
	
	cBullet* p = cBullet::Create();
	g_pBulletManager->Fire(cVector2(0, 0), cVector2(0, 0), vecTarget);
	g_pBulletManager->AddBullet(p);
	//p->Release();
}
